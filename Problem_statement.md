...
> **IMPORTANT!** This settings screen is mandatory. Teams that do not implement it will be disqualified, since we need it to test your system during evaluation.
...
i. **Accuracy (A ∈ [0, 1]):** Computed as `PassedTests / TotalTests` on our evaluation tasks
...
iv. **Penalty Exponent (ε):** Set to **2.5**, wherein you will be heavily penalized if the token cost exceeds the maximum cost allowed as per the constraints
v. **Reference Constraints:** - $C_{base}$ = $0.15
      - $T_{base}$ = 1320 seconds
      - $w_T$ = 0.35 and $w_C$ = 0.65 - Note that the base time and costs given are not targets. Just baseline reference costs.
...
ii. **Trade-off analysis**: 1. Did you explain why you made key architectural decisions, and why you rejected other approaches? 2. Did you actually try and compare more than one approach for any core component, instead of going with the first idea that worked?
...
5. **On involvement of Y26s:** In past competitions, we have observed a recurring issue where freshers do not actively participate in problem solving and are merely coached on specific sections for the presentation. To prevent this deceptive practice, presentations will now feature two distinct Q&A sessions: a general session for the entire team, and a dedicated session exclusively for Y26 students. The Y26 Q&A will go beyond generic concepts to test specific, implementation-based knowledge. If it becomes evident that the freshers did not actively contribute to building the project from scratch, the team will face severe penalties. We shall not reveal more on the exact pattern of the Y26 Q&A but broadly they will be tested on their fundamentals, contribution and participation in the PS.

---

## Team Composition

1. **Team Composition:** Each team should have **8 members**, with **at least 3 Y26s** and **3 Y25s**.

---

_For Any Queries, The Pool Captains and PS Leads are encouraged to ask in the Discord channel._
